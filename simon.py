...
def draw_normal_game_state():
  ...
  if get_global_game_state() == GS_INTRO:
    ...
    screen.draw.text(f"{PLAYERS[0].game.get_sequence_len(0)}", center=((WIDTH / 4) + PADDING, (HEIGHT / 2) + (y_start + (PADDING * 8))), fontsize=170, shadow=(1, 1), scolor="#202020", color="#FFFFFF")
    #screen.draw.text(f"{PLAYERS[1].game.get_sequence_len(1)}", center=((WIDTH / 4) + PADDING, (HEIGHT / 2) + (y_start + (PADDING * 8))), fontsize=170, shadow=(1, 1), scolor="#202020", color="#FFFFFF")
